...
class RiotLinkService {
  ...
  static const String _riotKey = String.fromEnvironment(
    'RIOT_KEY',
    defaultValue: 'RGAPI-808b2954-c656-41d6-a3b4-9c7f7272a384',
  );

  // 'auto' infers americas|europe|asia; or force a value via --dart-define
  static const String _routingOverride = String.fromEnvironment(
    'RIOT_ROUTING',
    defaultValue: 'auto',
  );
  ...
  Future<void> connectLoL({
    ...
    if (acct == null || routingUsed == null) {
      throw Exception(
        'Riot account not found on any cluster. Check Username/Tag.',
      );
    }
  ...
  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  // NEW: Save match-by-match performance summaries for dashboard charts
  //
  // Writes to:
  // Player/{uid}/linkedGames/lol/matches/{matchId}
  //
  // Does NOT change roleStats (AI stays the same).
  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

  double _calcLolScore({
    required bool win,
    required double kda,
    required double csPerMin,
    required double goldPerMin,
  }) {
    // Simple + understandable score, 0..100
    // Win matters, then KDA + farming + gold tempo.
    final winPts = win ? 40.0 : 0.0;

    // KDA: 0..6 -> 0..25
    final kdaPts = (kda.clamp(0.0, 6.0) / 6.0) * 25.0;

    // CS/min: 0..10 -> 0..20
    final csPts = (csPerMin.clamp(0.0, 10.0) / 10.0) * 20.0;

    // Gold/min: 200..500 -> 0..15
    final gpmNorm = ((goldPerMin - 200.0) / 300.0).clamp(0.0, 1.0);
    final gpmPts = gpmNorm * 15.0;

    return (winPts + kdaPts + csPts + gpmPts).clamp(0.0, 100.0);
  }

  Future<void> saveLolMatchSummaries({
    required String playerId,
    int maxMatches = 20,
    Duration freshness = const Duration(hours: 12),
    bool forceRefresh = false,
    ...
    if (!forceRefresh) {
      final last = (map['lastMatchesFetchedAt'] as Timestamp?)?.toDate();
      if (last != null && DateTime.now().difference(last) < freshness) {
        // ignore: avoid_print
        print('Matches fresh; skipping. Use forceRefresh:true to override.');
        return;
      }
    }

    // 1) Get match IDs
    var ids = await _getMatchIds(
      region,
      puuid,
      maxMatches,
    ...
    // ignore: avoid_print
    print(
      'MATCHES: got ${ids.length} ${rankedOnly ? "ranked" : "any"} match IDs',
    );

    if (ids.isEmpty && allowNonRankedIfEmpty) {
      // ignore: avoid_print
      print('MATCHES: no ranked matches; trying ANY queue…');
      ids = await _getMatchIds(region, puuid, maxMatches, rankedOnly: false);
      // ignore: avoid_print
      print('MATCHES: got ${ids.length} ANY-queue match IDs');
    }

    if (ids.isEmpty) return;

    final matchesCol = lolRef.collection('matches');

    const perReqDelay = Duration(milliseconds: 1200);
    WriteBatch batch = db.batch();
    int ops = 0;

    final max = ids.length > maxMatches ? maxMatches : ids.length;

    for (var i = 0; i < max; i++) {
      final matchId = ids[i];
      // ignore: avoid_print
      print('MATCHES: fetching ${i + 1}/$max id=$matchId');
      ...
      try {
        final res = await _withRetry(
          () => http.get(url, headers: {'X-Riot-Token': _riotKey}),
        );
        m = jsonDecode(res.body) as Map<String, dynamic>;
      } catch (e) {
        // ignore: avoid_print
        print('MATCHES: fetch failed ($matchId): $e');
        await Future.delayed(perReqDelay);
        continue;
      }

      final info = (m['info'] as Map?)?.cast<String, dynamic>() ?? const {};
      final parts = (info['participants'] as List?) ?? const [];

      final meDyn = parts.cast<Map>().firstWhere(
        (p) => p['puuid'] == puuid,
        orElse: () => {},
      );
      ...
      // Match role mapping (same as your seed builder)
      final raw =
          ((me['teamPosition'] ?? me['individualPosition'] ?? '') as String)
              .toUpperCase();
      String role;
      switch (raw) {
        case 'UTILITY':
        ...
        default:
          role = 'unknown';
          break; // keep match doc even if unknown
      }

      final bool win = (me['win'] as bool?) == true;
      final int kills = (me['kills'] ?? 0) as int;
      final int deaths = (me['deaths'] ?? 0) as int;
      final int assists = (me['assists'] ?? 0) as int;

      final int cs =
          ((me['totalMinionsKilled'] ?? 0) as int) +
          ((me['neutralMinionsKilled'] ?? 0) as int);

      final int gold = (me['goldEarned'] ?? 0) as int;

      // duration: can be int or num
      final int durationSec = (info['gameDuration'] is int)
          ? (info['gameDuration'] as int)
          : ((info['gameDuration'] as num?)?.toInt() ?? 0);

      // timestamps are ms
      final int startMs =
          (info['gameStartTimestamp'] ?? info['gameCreation'] ?? 0) is int
          ? (info['gameStartTimestamp'] ?? info['gameCreation']) as int
          : ((info['gameStartTimestamp'] ?? info['gameCreation']) as num?)
                    ?.toInt() ??
                0;

      final double minutes = durationSec > 0 ? (durationSec / 60.0) : 1.0;

      final double kda = deaths == 0
          ? (kills + assists).toDouble()
          : (kills + assists) / deaths;

      final double csPerMin = cs / minutes;
      final double goldPerMin = gold / minutes;

      final double score = _calcLolScore(
        win: win,
        kda: kda,
        csPerMin: csPerMin,
      ...
      batch.set(matchesCol.doc(matchId), {
        'matchId': matchId,
        'timestamp': startMs > 0
        ...
        'deaths': deaths,
        'assists': assists,
        'kda': double.parse(kda.toStringAsFixed(2)),
        'cs': cs,
        'csPerMin': double.parse(csPerMin.toStringAsFixed(2)),
        'gold': gold,
        'goldPerMin': double.parse(goldPerMin.toStringAsFixed(2)),
    ...
    await lolRef.set({
      'lastMatchesFetchedAt': FieldValue.serverTimestamp(),
    }, SetOptions(merge: true));

    // ignore: avoid_print
    print('FIRESTORE: wrote matches for $playerId');
  }
  ...
  Future<List<_RoleSeed>> _buildSeedsFromMatches(
    ...
    for (var i = 0; i < max; i++) {
      ...
      final url = Uri.parse(
        'https://$routing.api.riotgames.com/lol/match/v5/matches/$mid',
      );
...
// accumulators & struct
class _Acc {
  int games = 0, wins = 0, k = 0, d = 0, a = 0, cs = 0, gold = 0;
}
