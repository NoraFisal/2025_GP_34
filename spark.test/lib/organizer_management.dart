...
class _OrganizerManagementPageState extends State<OrganizerManagementPage> {
  ...
  Future<void> _load() async {
    ...
    _photoBase64 = (data['ProfilePhotoBase64'] ?? '').toString();
    _photoUrl = (data['ProfilePhoto'] ?? '').toString();
  ...
  Future<void> _save() async {
    if (!_formKey.currentState!.validate()) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('اكمل الحقول المطلوبة')));
      return;
    }
    ...
    try {
      ...
      await _docRef!.set({
        ...
        'ProfilePhotoBase64': photoToSave,
        'updatedAt': FieldValue.serverTimestamp(),
      }, SetOptions(merge: true));
  ...
  Widget build(BuildContext context) {
    ...
    return BgScaffold(
      ...
      // ✅ حطينا Positioned داخل Stack كجزء من body
      body: Stack(
        ...
        children: [
          ...
          // ✅ الهاندل الجانبي الخاص بالأورغنايزر
          const Positioned(
            left: 0,
